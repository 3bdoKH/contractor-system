...
import { Search, Plus, X, AlertCircle, Truck } from 'lucide-react';
...
const emptyForm: NewSupplierForm = { name: '', phone: '', address: '', notes: '' };
...
export default function Suppliers() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (query.trim()) {
        window.api.suppliers.search(query).then(setSuppliers);
      } else {
        loadSuppliers();
      }
    }, 300);
  ...
  function getSupplierStatusClass(s: Supplier) {
    const remaining = s.total_invoiced - s.total_paid;
    if (s.total_invoiced === 0) return 'bg-slate-100 text-slate-600';
    if (remaining <= 0) return 'bg-emerald-100 text-emerald-800';
    if (s.total_paid > 0) return 'bg-amber-100 text-amber-800';
    return 'bg-red-100 text-red-800';
  }

  function getSupplierStatusLabel(s: Supplier) {
    const remaining = s.total_invoiced - s.total_paid;
    if (s.total_invoiced === 0) return 'لا توجد فواتير';
    if (remaining <= 0) return STATUS_LABELS.paid;
    if (s.total_paid > 0) return STATUS_LABELS.partial;
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-900 mb-1">
            {query ? 'لا توجد نتائج' : 'لم يتم إضافة موردين بعد'}
          </h3>
          <p className="text-slate-500 max-w-sm">
            {query ? 'جرب البحث باسم مختلف أو تأكد من صحة رقم الهاتف' : 'ابدأ بإضافة أول مورد لك بالنقر على زر "مورد جديد" في الأعلى'}
          </p>
        </div>
      ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">المستحق</p>
                      <p className={`font-black text-sm ${remaining > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                        {formatCurrency(remaining)}
                      </p>
                    </div>
                    <span className={`text-[11px] font-bold px-3 py-1 rounded-full ${getSupplierStatusClass(s)}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4" dir="rtl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">إضافة مورد جديد</h2>
              <button
