...
export default function EditInvoice() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      window.api.invoices.getById(invId),
    ]).then(([cust, merch, inv]) => {
      if (!cust || !inv) { navigate(`/customers/${customerId}`); return; }
      setCustomer({ name: cust.name });
      setMerchandise(merch);
      setDate(inv.date);
      setNotes(inv.notes || '');
      setRows(inv.items.map((item: any) => ({
        id: Math.random().toString(36).slice(2),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await window.api.invoices.update(invId, {
        date: date.trim(),
    ...
    <div className="p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  return (
                    <tr key={row.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      <td className="px-3 py-2 text-center">
                        <button type="button" onClick={() => removeRow(row.id)} className="p-1 text-slate-300 hover:text-red-500 transition-colors rounded">
                          <Trash2 size={15} />
                        </button>
                      </td>
                      <td className="px-3 py-2">
                        {row.useCustom ? (
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => updateRow(row.id, { useCustom: false, custom_name: '' })} className="text-xs text-blue-500 hover:text-blue-700 whitespace-nowrap">اختر</button>
                          ...
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => updateRow(row.id, { useCustom: true, merchandise_id: null })} className="text-xs text-blue-500 hover:text-blue-700 whitespace-nowrap">مخصص</button>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/20 active:scale-[0.98]">
            {saving ? 'جاري الحفظ...' : 'حفظ التعديلات'}
          </button>
          <Link to={`/customers/${customerId}`} className="px-8 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-3 rounded-xl transition-colors text-center">إلغاء</Link>
        </div>
      </form>
    </div>
  );
