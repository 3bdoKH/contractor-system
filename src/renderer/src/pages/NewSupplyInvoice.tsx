...
export default function NewSupplyInvoice() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!date.trim()) { setError('يرجى إدخال تاريخ الفاتورة'); return; }
    ...
    if (validRows.length === 0) { setError('يرجى إضافة بند واحد على الأقل بكمية وسعر'); return; }

    setSaving(true);
    try {
      await window.api.supplyInvoices.create({
  ...
  return (
    <div className="p-6 space-y-6">
      <Link to={`/suppliers/${supplierId}`} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">
        {supplier?.name}<ArrowRight size={16} />
      </Link>

      <div>
        <h1 className="text-2xl font-bold text-slate-900">فاتورة توريد جديدة</h1>
        {supplier && <p className="text-slate-500 text-sm mt-1">{supplier.name}</p>}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-slate-100">
            <button type="button" onClick={() => setRows(rs => [...rs, newRow()])} className="flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700 font-medium">
              <Plus size={16} />إضافة بند
            </button>
            <h2 className="text-base font-semibold text-slate-900">بنود فاتورة التوريد</h2>
          </div>
            ...
            <table className="w-full text-sm">
                  ...
                  return (
                    <tr key={row.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                      <td className="px-3 py-2 text-center">
                        <button type="button" onClick={() => removeRow(row.id)} className="p-1 text-slate-300 hover:text-red-500 transition-colors rounded" title="حذف البند">
                          <Trash2 size={15} />
                        </button>
                      </td>
                      <td className="px-3 py-2">
                        {row.useCustom ? (
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => updateRow(row.id, { useCustom: false, custom_name: '' })} className="text-xs text-blue-500 hover:text-blue-700 whitespace-nowrap">اختر من القائمة</button>
                            <input type="text" value={row.custom_name} onChange={e => updateRow(row.id, { custom_name: e.target.value })} placeholder="اسم الصنف" className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-900" />
                          </div>
                        ) : (
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => updateRow(row.id, { useCustom: true, merchandise_id: null })} className="text-xs text-blue-500 hover:text-blue-700 whitespace-nowrap">صنف مخصص</button>
                            <select value={row.merchandise_id ?? ''} onChange={e => updateRow(row.id, { merchandise_id: Number(e.target.value) || null })} className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-900 bg-white">
                              <option value="">-- اختر صنف --</option>
                              {merchandise.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                            </select>
                          </div>
                        )}
                      </td>
              ...
              <tfoot className="bg-slate-100 border-t-2 border-slate-200">
                <tr>
                  ...
                  <td className="px-4 py-3 text-center font-bold text-lg text-slate-900">{formatCurrency(grandTotal)} ج.م</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white font-bold py-3 rounded-xl transition-colors shadow-sm text-base">
            {saving ? 'جاري الحفظ...' : 'حفظ فاتورة التوريد'}
          </button>
          <Link to={`/suppliers/${supplierId}`} className="px-8 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-3 rounded-xl transition-colors text-center">إلغاء</Link>
        </div>
      </form>
    </div>
  );
